...
theorem holds : Statement := by
  intro Validator BlockId Payload _ _ _ R hRa hRc P B upd C₀ hanc
  refine ⟨?_, ?_, ?_⟩
  · -- BN5a.
    intro U V₁ V₂ K₁ K₂ R₁ R₂
    have hrange : ∀ k, k < min K₁ K₂ → R₁.rangeLedger k = R₂.rangeLedger k := by
      intro k hkm
      have h := Agreement.holds Validator BlockId Payload R hRa P B upd C₀ hanc
        U V₁ V₂ K₁ K₂ R₁ R₂
