...
instance [DecidableEq BlockId] (blk : BlockId → Block Replica BlockId)
  ...
  decidable_of_iff
    ((∀ i ∈ b.refs, (blk i).round + 1 = b.round) ∧
      ...
      (0 < b.round → q Replica ≤ (creators blk b).card))
    ⟨fun h => ⟨h.1, h.2.1, h.2.2⟩,
      fun h => ⟨h.predecessor, h.distinct_creators, h.quorum⟩⟩
...
`ValidWrt` docstring. -/
theorem ValidWrt.refs_empty_of_round_zero
    {blk : BlockId → Block Replica BlockId} {b : Block Replica BlockId}
    (hv : ValidWrt blk b) (h0 : b.round = 0) : b.refs = ∅ := by
  by_contra h
